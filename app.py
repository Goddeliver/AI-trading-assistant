...
HTML="""
<!DOCTYPE html>
<html><head><title>GODIVAFX001 AI</title>
...
.logo{width:130px;height:130px;border-radius:50%;border:3px solid #d4af37}
#ans{white-space:pre-wrap;text-align:left;color:white;margin-top:15px}
</style></head>
<body>
...
def ask_ai():
    ...
    prompt="You are GODIVAFX001 AI. Gold "+str(price)+". User: "+str(q)+". Reply BUY/SELL/WAIT + confidence + reason + entry SL TP."
    ...
    try:
        ...
        j=r.json()
        if "choices" not in j:
            return jsonify({"answer":"Groq error: "+str(j)})
